...
public class FileUtil {
	public static MultipartRequest upload(HttpServletRequest req, String saveDirectory) {
		/*
		 * 매개변수1 : 사용자의 요청 정보가 저장된 request내장객체 매개변수2 : 파일이 저장된 서버의 물리적 경로 그외 업로드할 파일의
		 * 최대용량, 인코딩방식, 파일명 중복처리를 위한 클래스를 통해 NultipartRequest객체를 생성한다. 객체생성이 완료되면 파일은
		 * 업로드된다.
		 */
	...
	/* 파일 다운로드 처리 : 원본파일명과 저장된파일명을 지정하여 파일 다운로드시
	 * 	파일명을 원본파일명으로 변경한다.
